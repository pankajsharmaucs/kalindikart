...
export default function UserDashboardPage() {
  ...
  useEffect(() => {
    const fetchUserDetails = async () => {
      // 1. Get the identifier from localStorage (trying mobile first, then email)
      const identifier = localStorage.getItem('userMobile') || localStorage.getItem('userEmail');

      if (isLoggedIn && identifier) {
        try {
          // 2. Hit your updated API that handles either email or mobile as "identifier"
          const res = await fetch(`/api/user/by-mobile?identifier=${encodeURIComponent(identifier)}`);
          const data = await res.json();

          if (data.exists) {
            setUserData(data.user);
          } else {
            console.warn("User exists in local session but not in DB");
          }
        } catch (err) {
    ...
    fetchUserDetails();
  }, [isLoggedIn, authLoading]); // Removed authUser to prevent infinite loops if authUser updates via setUserData
    ...
    <div className="dashboard-wrapper" style={{ backgroundColor: brand.bg, minHeight: '100vh', paddingBottom: '80px' }}>
      ...
      <div className="container-fluid px-2 mt-3">
        ...
        <div className="row g-2 mb-3">
          ...
          <QuickLink icon={<FaHouseUser color={brand.primary} />} label="Address" link="/user/dashboard/profile" />
